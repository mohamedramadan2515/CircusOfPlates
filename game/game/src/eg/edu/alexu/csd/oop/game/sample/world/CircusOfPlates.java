...
public abstract class CircusOfPlates implements World, Observer {
	...
	private int addClown(int widthOfClown) {
		...
		FallingObject rightPlate = platesFactory.getRandomPlate(widthOfClown + clown.getWidth() * 1 / 45,
				(int) (height - clown.getHeight()), 2);
		FallingObject leftPlate = platesFactory.getRandomPlate(widthOfClown + clown.getWidth() * 7 / 10,
				(int) (height - clown.getHeight()), 2);
		control.add(clown);
	...
	public boolean refresh() {
		...
		for (int j = 0; j < look.size(); j++) {
			moving.remove(look.get(j));
			if (!(look.get(j) instanceof StaticObject)) {
				addPlate();
			}
		}
